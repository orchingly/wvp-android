...
//    private static final int GESTURE_MODE_ONLY_CALENDAR = 1;
    ...
    public CalendarLayout(Context context, AttributeSet attrs) {
        ...
        Log.w("CUSTOM_LAYOUT", "CalendarLayout: init");
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        Log.w("CUSTOM_LAYOUT", "onTouchEvent: event");
//        if (true)
//            return false;
    ...
    private static final String TAG = "CUSTOM_LAYOUT";
    @Override
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_MOVE) {
             ...
             */
            //增大点击概率滑动2像素
            if (dy > 2 && mContentView.getTranslationY() == -mContentViewTranslateY) {
                boolean isS = isScrollTop();
                boolean shouIn = shouldInterceptByInnerView(ev);
                Log.d(TAG, "dispatchTouchEvent: isScrollTop " + isS +", shouldInterceptByInnerView " + shouIn);
                if ( isS && shouIn) {
                    requestDisallowInterceptTouchEvent(false);//父View向子View拦截分发事件
                    Log.w("CUSTOM_LAYOUT", "dispatchTouchEvent: isScrollTop" );
                    return super.dispatchTouchEvent(ev);
                }
            }
            else if (dy < -2 && mContentView.getTranslationY() != -mContentViewTranslateY){
                Log.d(TAG, "dispatchTouchEvent: 拦截,上滑,日历未折叠");
                requestDisallowInterceptTouchEvent(false);//父View向子View拦截分发事件
                return super.dispatchTouchEvent(ev);
            }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        Log.w("CUSTOM_LAYOUT", "onInterceptTouchEvent: ");
        if (isAnimating) {
            return true;
        }
        if (mGestureMode == GESTURE_MODE_DISABLED) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (dy > 0 && mContentView.getTranslationY() == -mContentViewTranslateY
                    ...
                    if (!shouldInterceptByInnerView(ev)){
                        Log.d(TAG, "onInterceptTouchEvent: shouldInterceptByInnerView intercept");
                        return false;
                    }
                    else {
                ...
                Log.d(TAG, "onInterceptTouchEvent: dx= " +dx +", dy= " + dy);
                ...
                if (Math.abs(dy) > Math.abs(dx) ) { //纵向滑动距离大于横向滑动距离,拦截滑动事件
                    if ((dy > 0 && mContentView.getTranslationY() <= 0)
                        ...
                        mLastY = y;
                        Log.d(TAG, "onInterceptTouchEvent: intercept by 纵向滑动距离大于横向滑动距离");
                        return true;
                    }
                }
                break;
    ...
    /**
     *
     * @return 当内部嵌套RecyclerView true:滑动事件位于RecyclerView内且滑动到顶部, false:其他情况
     */
    private boolean shouldInterceptByInnerView(MotionEvent ev) {
        float x = ev.getX();
        float y = ev.getY();

//        boolean canScroll = recordList.canScrollVertically(-1);
        Log.d(TAG, "shouldInterceptByInnerView:computeVerticalScrollOffset  " + recordList.computeVerticalScrollOffset());
        //点击事件必须落在控件内
        return isTouchPointInView(recordList, (int) x, (int) y)
                //控件已经滑动到顶部
                && recordList.computeVerticalScrollOffset() == 0;
    }
    ...
    private boolean isTouchPointInView(View v, int x, int y){
        ...
        Log.d(TAG, "isTouchPointInView: x="+x+", y="+y+", l="+left+", r="+right+", t="+top+", b="+bottom);
    ...
    public void setRecyclerView( RecyclerView dayRecordList) {
        recordList = dayRecordList;
    }
